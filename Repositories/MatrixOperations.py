...
class MatrixOperations:
    def matrixSum(self, matrixOne: np.ndarray, matrixTwo: np.ndarray):
        """Suma dos matrices"""
    ...
    def matrixMinus(self, matrixOne: np.ndarray, matrixTwo: np.ndarray):
        """Resta dos matrices"""
        rowsOne = len(matrixOne)
        colsOne = len(matrixOne[0])
        rowsTwo = len(matrixTwo)
    ...
    def matrixMult(self, matrixOne: np.ndarray, matrixTwo: np.ndarray):
    ...
    def matrixScalarMult(self, matrix: np.ndarray, scalar: float):
    ...
    def matrixTranspose(self, matrix: np.ndarray):
    ...
    def matrixDeterminant(self, matrix: np.ndarray):
    ...
    def matrixInverse(self, matrix: np.ndarray):
    ...
    def matrixRank(self, matrix: np.ndarray):
    ...
    def matrixEigenValues(self, matrix: np.ndarray, maxIter=100, tol=1e-10):
    ...
    def getMatrixOperations(self, matrixArray):
        """Retorna las operaciones disponibles para una matriz"""
        if matrixArray == None:
            raise ValueError("No es valida una matriz nula")
        matrixSum = self.matrixSum(matrixArray, matrixArray)
        matrixMinus = self.matrixMinus(matrixArray, matrixArray)
        matrixMult = self.matrixMult(matrixArray, matrixArray)
        matrixNumsMult = self.matrixScalarMult(matrixArray, 2)
        matrixT = self.matrixTranspose(matrixArray)
        
        operations = "+, -, *, T(Transpuesta)"
        
        if len(matrixArray) == len(matrixArray[0]):
            matrixDet = self.matrixDeterminant(matrixArray)
            matrixInv = self.matrixInverse(matrixArray)
            matrixRank = self.matrixRank(matrixArray)
            matrixEigen = self.matrixEigenValues(matrixArray)
            operations = operations+", det(Determinante), inv(Inversa), rank(Rango), eigen(Valores propios)"
